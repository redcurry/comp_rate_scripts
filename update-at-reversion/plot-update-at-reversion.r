...
plot(0, 0, type = 'n', las = 1, xaxt = 'n', xlim = c(0, 1), ylim = c(0, 850),
  ...
  ylab = 'Time at which reversion appears\nfor runs that reverted (gens)')
...
# Temporary: Do certain treatments
data = data[data$Ancestor == 1 & data$Mutant == 2,]

for (mut_fit in c(0.1, 0.3, 0.7, 0.9))  # Skipping 0.5 for now
{
